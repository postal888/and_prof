...
class YouTubeTranscriptFetcher(
    ...
    private fun mergeSegments(segments: List<TranscriptSegment>): List<SubtitleLine> {
        ...
        val buffer = mutableListOf<TranscriptSegment>()

        fun flush() {
            if (buffer.isEmpty()) return
            val text = buffer.joinToString(" ") { it.text }.trim()
            val start = buffer.first().offsetSec
            var end = buffer.last().let { it.offsetSec + it.durationSec }
            if (end <= start) end = start + 0.3f
            out += SubtitleLine("line-${out.size + 1}", text, start, end)
            buffer.clear()
        }
        ...
        for (segment in segments) {
            ...
            buffer += segment
            val joinedLen = buffer.joinToString(" ") { it.text }.length
            if (Regex("[.!?…:;]\\s*$").containsMatchIn(txt) || joinedLen > 220) flush()
        }
        flush()
        return out
    }
