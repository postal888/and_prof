...
fun CreateStudySetScreen(
    ...
    viewModel: CreateStudySetViewModel = viewModel(
        factory = CreateStudySetViewModelFactory(repository, selectionStore),
    ),
) {
    ...
    Column(
        ...
        Row(
            ...
            Text("Новый набор", style = MaterialTheme.typography.headlineMedium, color = PpHeading)
        }
        ...
        OutlinedButton(
            ...
            Text("Выбрать слова", color = PpAccent)
        }

        Text(
            text = "Выбрано: ${state.selectedWords.size} слов",
                ...
                PortCard(modifier = Modifier.fillMaxWidth()) {
                    Row(
                        ...
                        IconButton(onClick = { viewModel.removeWord(word.id) }) {
                            Icon(Icons.Default.Close, contentDescription = "Убрать", tint = PpTextMuted)
                        }
                    }
                }
            }
